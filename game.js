...
input_queue : [], // received messages are stored here until the system is properly initialized.
...
login: function(opts)
{
	...
	socket.onmessage = function(evt) {
		...
		if('challenge' in json)
		{
			...
			{
				if(World.callbacks.login_failed)
					World.callbacks.login_failed();
				return;
			}
		...
		else if('success' in json)
		{
			...
			if(success)
			{
				...
				World.init();
				if(World.callbacks.login_successful)
					World.callbacks.login_successful();
			}
			else
				if(World.callbacks.login_failed)
					World.callbacks.login_failed();
...
queueImage : function(src)
{
	...
	img.onload = function()
	{
		...
		if(World.preload.completion_counter == 
...
startFetch : function()
{
	...
	{
		if(!World.preload.images.hasOwnProperty(key))
			continue;
...
loadLevel: function()
{
	...
	manager.addTemplate(
		['_cursor','plethora.png',false,0,1,1,0,[[5,1]]]
	);
	
	// Add builtin proxys
	World.cursor = manager.addProxy(
		[0, '_cursor', 0,0,1]
	);
...
processFrame: function(ms)
{
	...
	var txt = 'Mouse at ('+Key.mouse_x+', '+Key.mouse_y+')';
...
updateCursor : function()
{
	...
	if(res.length > 0)
	{
		...
		if(hitpos.x >= p.x && hitpos.x < p.x+p.template.bx && 
		   hitpos.y >= p.y && hitpos.y < p.y+p.template.by &&
		   1 /*Math.abs(hitpos.z-(p.z+p.template.bz)) < 0.001*/)
		{
			...
			if(Key.get(MOUSE_LEFT) && Key.changed(MOUSE_LEFT))
			{
				...
				w.send({mouse: [w.cursor.x, w.cursor.y, w.cursor.z]});
			}
		}
	}
},
...
onMessage: function(evt)
{
	...
	{
		// Global-message
		World.log('Server message: ' + json.gmesg);
	}
}
