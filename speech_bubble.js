var SpeechBubble = function(name){
	...
	this._hitEdge = {'left': false, 'right': false, 'top': false, 'bottom': false};
...
SpeechBubble.prototype._render = function(){
	...
	if ( this._lines[0].isName )
		this._context.font = 'bold '+this._fontSize + 'px sans-serif';
	else
		this._context.font = this._fontSize + 'px sans-serif';

	this._size.w = 0;
	for ( var i=0; i<this._lines.length; ++i ){
		var width = this._context.measureText(this._lines[i]).width;
		if ( width > this._size.w )
			this._size.w = width;

		if ( this._lines[i].isName )
			this._context.font = this._fontSize + 'px sans-serif';
	}
	this._size.w += 10;
	...
	for ( var i=0; i<this._lines.length; ++i){
		this._context.fillText(
			this._lines[i].text,
			this._textMargin,
			this._textMargin + (this._fontSize + this._lineMargin) * i,
			this._size.w - this._textMargin*2
		);
	...
	if ( this._lines[0] && this._lines[0].isName )
		this._lines.shift();
};
...
SpeechBubble.prototype.uncollide = function(){
	for ( var i=0; i<SpeechBubble.instances.length; ++i ){
		...
		if ( this._hitEdge.left && !other._hitEdge.left )
			continue;
		if ( this._hitEdge.right && !other._hitEdge.right )
			continue;

		if (
			this._pos.x > other._pos.x + other._size.w ||
			this._pos.x + this._size.w < other._pos.x  ||
			this._pos.y > other._pos.y + other._size.h ||
			this._pos.y + this._size.h < other._pos.y
		)
			continue;

		if ( this._hitEdge.right && other._hitEdge.right ){
			if ( this._hitEdge.top )
				other._pos.y = this._pos.y + this._canvas.height;
			else
				this._pos.y = other._pos.y - this._canvas.height;
			this._reRender = true;
			continue;
		}

		if ( this._pos.x <= other._pos.x )
			this._pos.x = other._pos.x - this._size.w;
	...
	if ( this._renderPos.x > this._pos.x )
		this._renderPos.x = Math.max(this._pos.x, this._renderPos.x - xSpeed);
	else if ( this._renderPos.x < this._pos.x )
		this._renderPos.x = Math.min(this._pos.x, this._renderPos.x + xSpeed);
	else
		xMoved = false;

	if ( this._renderPos.y > this._pos.y )
		this._renderPos.y = Math.max(this._pos.y, this._renderPos.y - ySpeed);
	else if ( this._renderPos.y < this._pos.y )
		this._renderPos.y = Math.min(this._pos.y, this._renderPos.y + ySpeed);
	else
		yMoved = false;
