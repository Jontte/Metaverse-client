...
ProxyManager.prototype.addProxy = function(json)
{
	...
	else
	{
		...
		if(id != 0)
			this.proxies_id[proxy.id] = proxy;
	...
	if(new_object)
	{
		...
		proxy.addCallback('label', function(p, old, value){
			if(p.bubble == null)
				p.bubble = new SpeechBubble(value);
			else
				p.bubble.name(value)
		});
	}
...
ProxyManager.prototype.rebuildGraph = function()
{
	...
	for(var i = 0; i < erased.length; i++)
	{
		...
		{
			if(prev != null)
				prev.insert_after(erased[i]);
			else
				this.proxies.push_front(erased[i]);
		}
	}
}
...
ProxyManager.prototype.speechBubble = function(id, message)
{
	if(id in this.proxies_id)
	{
		...
		if(p.bubble == null)
			p.bubble = new SpeechBubble(('label' in p.ext)?p.ext.label:null);
